...
\begin{sloppypar}
As is well known, standard present-day \ili{Swedish} differs from all of the other (North) \ili{Germanic} languages in only allowing the order \isi{verb particle}–object, cf. \REF{ex:lalu:1a} and \REF{ex:lalu:1b} (see e.g. \citealt{Svenonius1996, Svenonius2003, Svenonius1996}; \citealt{Toivonen2003}; \citealt{Lundquist2014Active}).\footnote{Unless otherwise indicated, all examples are from present-day \ili{Swedish}.}
\end{sloppypar}
...
This paper is concerned with the development of the strict order \isi{particle}–object in the history of \ili{Swedish}. The article has two objectives. First, we aim to give an accurate description of the change in word order with respect to verb particles and direct objects. The focus is on the 17\textsuperscript{th}--19\textsuperscript{th} centuries, which is the period where we see the most rapid change. Secondly, we discuss different ways of modelling the change in a generative framework. We show that the change in \isi{particle} constructions cannot be a direct consequence of the shift from \isi{OV order} to \isi{VO order} in the history of \ili{Swedish}. We argue that it should not be analyzed in terms of a change in \isi{headedness} or in available argument positions in the verb phrase, nor can we link it to a general change in \isi{pronominal object} shift. Instead, we propose that the change is best described as a \isi{reanalysis} of the \isi{particle}. We propose that in older \ili{Swedish}, the \isi{particle} is a phrasal modifier of a result-encoding phrase, whereas in present-day \ili{Swedish} it is the head of the said phrase. The change can thus be captured by the so-called \isi{Head Preference Principle} (\citealt{van_Gelderen2004}). We will see that not all particle-like elements behave in the same way in older \ili{Swedish}, and that not all elements or contexts change at the same time. Most evidently, in the context of a PP, directional adverbs do not behave like particles in older \ili{Swedish}, but in present-day \ili{Swedish} they do. Overall, the category of particles appears to be syntactically more homogeneous in present-day \ili{Swedish} than in \ili{Swedish} before the middle of the 17\textsuperscript{th} century. 



The structure of the article is as follows. In \sectref{sec:lalu:2}, we give a brief description of the category of \isi{verb particle}, in the sense it is used in the literature on the \ili{Germanic} languages. We also go through the characteristics of particles in present-day \ili{Swedish} and give an overview of what is included in our study of older \ili{Swedish}. \sectref{sec:lalu:3} introduces the historical corpus, and in §\ref{sec:lalu:4} and §\ref{sec:lalu:5} we present the results from the corpus study. In \sectref{sec:lalu:6}, we discuss possible analyses of the change. We discuss the connection between the change in word order in \isi{particle} constructions and the shift from \isi{OV order} to \isi{VO order}, and possible changes in \isi{object shift} and VP-internal argument positions, but propose that the change is best understood as a change in the properties of the \isi{particle}. \sectref{sec:lalu:7} briefly discusses some recent developments and concludes the paper.
...
We will return to the formal analysis of particles in \sectref{sec:lalu:6} below. For now, the reader should keep in mind the following properties of verb particles: they behave like intransitive prepositions, they tend to induce or modify endpoints in event descriptions, and they behave like independent words. In the next section we will look at diagnostics for particles in present-day \ili{Swedish}.


\subsection{Identifying particles in present-day Swedish}\label{sec:lalu:2.2}

It is often not evident how verb particles should be identified in corpora (see e.g. \citealt{LarssonRoxendal2020} for discussion). Words from different classes (e.g. prepositions, adverbs, and adjectives) can be used as particles. Only in a few exceptional cases can particles be lexically identified; in present-day \ili{Swedish}, this is the case for instance with \textit{ihjäl} ‘to death’ which is only used as a \isi{verb particle}, or \textit{an} ‘to’ which is always either a \isi{particle} or a \isi{prefix}.


\begin{sloppypar}
There are, however, some prosodic and syntactic diagnostics to distinguish \isi{particle} constructions from constructions with verb\,+\,PP, or verb\,+\,adjectival small clause. In present-day \ili{Swedish}, word order is often used as a diagnostic: since a \isi{particle} always precedes the object, examples like those in \REF{ex:lalu:9} must involve AdvPs or APs.
\end{sloppypar}
...
Another way of identifying particles in present-day \ili{Swedish} is that they obligatorily incorporate into participles (even in verbal passives; see \citealt{Lundquist2014Passives} and references therein). Consider the contrast between the canonical \isi{periphrastic passive} in \REF{ex:lalu:14a} and the pseudopassive in \REF{ex:lalu:14b}.
...
Word order, \isi{prosody}, constituency, and \isi{incorporation} can thus be used to identify particles in present-day \ili{Swedish}, at least in many cases. We return to some more problematic cases in the next section, where we discuss how we can identify particles in older \ili{Swedish} texts. 


\subsection{The particles in this study}\label{sec:lalu:2.3}

None of the diagnostics discussed above can be directly applied to historical corpus data. Rather, to identify particles in older, written texts, we have to rely partly on our intuitions about the properties of the present-day \ili{Swedish} translational equivalents of the relevant particle-like elements we find in the corpus. In this study, we have included constructions that would have the word order and \isi{prosody} of present-day \ili{Swedish} particles. Moreover, we have used the possibility of an object \textit{preceding} a prepositional element to identify particles in older \ili{Swedish} (as in present-day \ili{Icelandic} and \ili{Norwegian}): in older \ili{Swedish}, the order object–\isi{particle} is possible, whereas the order \isi{complement}–preposition is not.\footnote{There is one systematic exception, involving \textit{r}{}-pronouns (e.g. \textit{der} ‘there’), which precede prepositions in \ili{Old Swedish} \citep{Delsing2014Svenskans}. It is also possible that complex prepositions like \textit{emot} ‘toward’ could follow their complements in older \ili{Swedish} (Falk p.c.) – this would make it particularly difficult to distinguish prepositional from \isi{particle} uses.}  Examples with the order object–preposition/\isi{particle} are therefore included among the \isi{particle} constructions in the historical study.\footnote{In
    15\textsuperscript{th}--18\textsuperscript{th} century \ili{Swedish}, the preposition \textit{till} ‘to’ can follow a \isi{benefactive} object. Examples like (i) are included among the \isi{particle} constructions.
...
In the present study, we have followed traditional grammars and also included cases with prepositional elements that take ground arguments among the particles, if other diagnostics (e.g. \isi{prosody}) point toward a \isi{particle} analysis. In this way, we can investigate whether these constructions pattern with other particles in the history of \ili{Swedish}. We will, however, code them as “ground” particles and take their special semantics into consideration in the analysis.
...
The examples in \REF{ex:lalu:17} do not necessarily all have the same structure. In present-day \ili{Swedish}, \REF{ex:lalu:17a} requires an analysis of \textit{upp} as a \isi{particle}, whereas in \REF{ex:lalu:17b} the word order (object–\isi{adverb}) rules out that analysis. In older \ili{Swedish}, on the other hand, \REF{ex:lalu:17b} could also be analyzed as involving a \isi{particle}. We have therefore included both cases corresponding to \REF{ex:lalu:17a} and \REF{ex:lalu:17b} in the study, but we have annotated them so that they can easily be treated separately. As we will see in the following, the word order in \REF{ex:lalu:17b}, with the object preceding the \isi{particle} in the context of a PP, is the rule in older \ili{Swedish}, but gradually becomes less frequent.



\subsubsection{Modified particles} 
We have excluded a few cases with modified particles, like \REF{ex:lalu:18}. Here, the \isi{particle} necessarily follows the object, in (older) \ili{Swedish} as in the other \ili{Germanic} languages (but see \sectref{sec:lalu:7} below). Examples like these are rare in the historical texts, and they are uncommon even in the present-day corpora; we return to them briefly in \sectref{sec:lalu:7}. Modified particles are not included in the corpus study.
...
Simple \isi{reflexive} objects show a somewhat variable behaviour with respect to \isi{particle} placement in present-day \ili{Swedish}. We find at least three patterns, as exemplified in \REF{ex:lalu:19} below (see \citealt{Lundquist2014Reflexive} and references therein).
...
In these cases, objects have to precede particles in present-day \ili{Swedish}. The \isi{particle} does not carry \isi{particle stress}, and it cannot be incorporated into passives.



Finally, we have excluded \isi{particle} verbs with clausal complements, as clausal complements always followed particles in older \ili{Swedish}, as they do in the other North \ili{Germanic} languages and \ili{English} (e.g., \textit{I found out that John won the race\slash\relax *I found that John had won the race out}.)



To summarize, we have largely followed traditional \ili{Swedish} grammars when identifying particles in older \ili{Swedish} texts. We have intentionally been rather liberal and included everything that would lend itself to a \isi{particle} analysis in present-day \ili{Swedish}, since this makes it possible for us to investigate how the \isi{particle} category should be delimited, rather than assuming an a priori stable category. However, we do not include constructions with \isi{reflexive} or clausal objects, nor examples where the \isi{particle} follows the object in present-day \ili{Swedish}. In addition to present-day \ili{Swedish} particles, we have included cases that behave like \isi{particle} constructions in older \ili{Swedish} (e.g., by allowing word order variation). This includes cases where present-day \ili{Swedish} has a \isi{prefixed verb}, e.g., older \ili{Swedish} \textit{billa någon in} for present-day \textit{inbilla någon} ‘make somebody believe’, or older \textit{gå någon an} for present-day \textit{angå någon} ‘concern somebody’.


\section{The corpus}\label{sec:lalu:3}


The study uses a corpus of 18 texts from the period ca. 1450–1849. Together with data from previous studies, one text (\textit{Didrik av Bern}, ca. 1450) represents \ili{Late Old Swedish}. The choice of this particular text is based on the results from \citet{Ljunggren1932}, who shows that it has a considerable number of particles that follow the non-finite verb; this allows us to investigate the order between \isi{particle} and object (independently of \isi{OV order}). In addition, the corpus includes several texts that are generally assumed to reflect the language of \isi{Central Sweden} (Stockholm, Uppsala, and surroundings) better than other texts from the same period, for instance \textit{Peder Swarts krönika} (1560), and the autobiography of Agneta Horn (1657). Kiöping’s travel description (published in 1674) and the edition by Salvius (1743) are versions of the same text, and therefore allow for good comparison. Finally, the corpus contains 13 plays from the period 1734 to 1849, taken from the corpus of \ili{Swedish} drama dialogue \citep{MarttalaStromquist2001}. As historical sources, these plays are particularly interesting and useful, since the authors often attempt to reflect the spoken language of their time. As we will see in \sectref{sec:lalu:5}, the plays can, however, be more or less liberal.
...
To sum up this brief description of the ordering of particles and objects in \ili{Swedish} up until the 17\textsuperscript{th} century, pronouns are placed before a \isi{particle}, whereas the placement of non-pronominal objects varies. There are, however, a couple of cases where what have here been included in the study of \isi{particle} constructions, on the basis of present-day diagnostics, should rather be analyzed as PPs. 
...
\lsptoprule
Text & \multicolumn{3}{l}{\isi{Particle}–pronoun} & \multicolumn{2}{c}{\isi{Particle}–DP} & \multicolumn{2}{c}{Total}\\
\midrule
\textit{Didrik} (ca. 1450) & 2/18 & (11\%) & & 41/97 & (42\%) & 43/115 & (37\%)\\
Swart (1560) & 3/26 & (12\%) & & 35/74 & (47\%) & 38/100 & (38\%)\\
...
We have not systematically investigated the ordering of particles and reflexives; recall from \sectref{sec:lalu:2} that there is still variation in the placement of reflexives in present-day \ili{Swedish}. However, we can note that from Horn onwards, it is possible to find examples of reflexives following the \isi{particle}:
...
\glt `if my manners do not befit everyone'      (Gyllenborg, b. 1679)\footnote{When referring to texts in the electronic corpus of \ili{Swedish} drama dialogue, we give only the author and year of birth, not the page number.} \\
\z
\ex\label{ex:lalu:38}
\ea
...
\begin{sloppypar}
In the 18\textsuperscript{th} and 19\textsuperscript{th} centuries, there is still variation in these contexts. Even in the youngest text in the corpus, there are examples of the order object–\isi{particle}\,+\,PP, where present-day \ili{Swedish} would require a different word order. One such example is given in \REF{ex:lalu:44}.
\end{sloppypar}
...
To sum up, we can identify three stages in the development of the \ili{Swedish} word order in \isi{particle} constructions:
...
\item[Post-change (authors born 1748–1818):] In the texts by authors born after 1748 (i.e., texts from the late 18\textsuperscript{th} century onwards) we find a system that looks like the present-day \ili{Swedish} system, with particles more or less obligatorily preceding objects. We still find at least two more or less systematic types of counterexamples: 1) particles appearing together with directional PPs; and 2) particles with atypical semantics, i.e. that neither provide an endpoint nor take a figure external argument (see \ref{ex:lalu:45}).
\end{description}
...
In this section, we discuss possible analyses of these changes. We will in turn look at what we see as the two main alternatives. Firstly, one could analyze the change in \isi{particle} constructions as being on a par with the change from OV to \isi{VO order}, i.e. either as a consequence of the \isi{headedness} of the \isi{particle} or in terms of argument shifts across the \isi{particle} head (as suggested for the development of \isi{VO order}, e.g. by \citealt{Petzell2011}). In an account along such lines, the word order change in \isi{particle} constructions would be another step toward a consistent \isi{head-initial} language. An alternative is to assume that \ili{Swedish} particles have become different, and that the word order change is a consequence of a \isi{reanalysis} of the \isi{particle}. 



In the following, we discuss these two possibilities in turn. In \sectref{sec:lalu:6.1}, we briefly compare the word order change in \isi{particle} constructions with the change from \isi{OV order} to \isi{VO order}. Although this alternative might seem initially appealing, we will see that it is problematic to assume object \isi{movement} across a verb phrase-internal head in 17\textsuperscript{th} and 18\textsuperscript{th} century \ili{Swedish}. Moreover, the analysis fails to account for the parallels between older \ili{Swedish} and present-day \ili{Norwegian}. In \sectref{sec:lalu:6.2}, we look more closely at pronominal argument shifts in older \ili{Swedish}, and we observe that the word order change in Early and \ili{Late Modern Swedish} is limited to constructions with a \isi{particle}: \isi{object shift} across negation is not affected. Finally, in \sectref{sec:lalu:6.3}, we will propose that the observed changes in \isi{particle} constructions are best understood as resulting from one single underlying change: a \isi{reanalysis} of particles from phrasal modifiers to heads in the verb phrase. By assuming that the \isi{particle} was a phrase in older \ili{Swedish}, we can more straightforwardly explain the word order variation. At the same time, this allows us to account for other changes in the properties of particles that took place during the same period, e.g. the loss of the possibility to combine \isi{adverbial} particles with a double object structure. Finally, the more recent change in constructions with \isi{particle}\,+\,PP follows from the same underlying change: in older \ili{Swedish}, the \isi{particle} modified the PP, whereas in present-day \ili{Swedish}, there is a strong preference for treating the \isi{particle} as a head. We will tentatively suggest that this also accounts for a drop in the use of modified particles.


\subsection{OV to VO and OP to PO}\label{sec:lalu:6.1}

The shift to strict \isi{particle}–object order partly overlaps with the shift from \isi{OV order} to \isi{VO order}. The change from \isi{OV to VO} had mainly taken place during the 13\textsuperscript{th} century (see e.g. \citealt{Delsing1999}), but OV structures increased in the late 15\textsuperscript{th} century, and residual \isi{OV word order} is not hard to find in the 16\textsuperscript{th} and 17\textsuperscript{th} century texts. It is perhaps tempting to view the two changes as one and the same, i.e., as a shift to a consistent \isi{head-initial} word order. 
...
We could in principle assume that \ili{Swedish} lost its landing positions within the verb phrase gradually, and that the lowest ones were available the longest. This would mean that the other North \ili{Germanic} languages (and \ili{English}) kept a low landing position in the verb phrase, a position either headed by or modified by the \isi{particle}. There is one crucial problem with such a proposal, and that concerns regular \isi{object shift}.\footnote{Erik Petzell (p.c.) points out that there is another problem with such a proposal, namely that in the loss of \isi{OV order}, short \isi{movement} of the object (giving the order V\textsubscript{aux}–object--V\textsubscript{main}) seems to disappear before long \isi{movement} (giving the order object--V\textsubscript{aux}–V\textsubscript{main}); see \citet{Petzell2012}.} A well-known difference between particles in \ili{Swedish} and the other North \ili{Germanic} languages is that particles block \isi{object shift} in \ili{Swedish}, but not in the other languages (see e.g. \citealt{Holmberg1986}; \citealt{Sells1998}). In \ili{Swedish}, a \isi{particle} behaves just like a verb inside the verb phrase; compare \REF{ex:lalu:47a} with \REF{ex:lalu:47b} and \REF{ex:lalu:47c}.
...
The restrictions on \isi{pronominal object} shift are the same in \ili{Swedish} as in the other North \ili{Germanic} languages, with the exception of shift across particles. In the other North \ili{Germanic} languages (exemplified with \ili{Norwegian} in \REF{ex:lalu:48}), a light object pronoun must shift across both the \isi{particle} and the sentence \isi{adverb} (in the context of verb \isi{movement}). \ili{Swedish} seems to have been like present-day \ili{Norwegian} up until the 18\textsuperscript{th} century (although the relevant examples are few); see the examples in \REF{ex:lalu:49}.
...
It seems clear from the results in \tabref{tab:lalu:4} that \isi{pronominal object} shift is (almost) obligatory in older \ili{Swedish}; as many as 90\% of the pronouns shift across negation. Although the number of examples is small in the individual texts, we can conclude that the placement of pronouns in relation to negation is stable during the period.
...
Now, present-day \ili{Swedish} is different, and we have argued that the change should not be understood as a change in branching or argument shifts. Instead, we propose that the \isi{particle} has been reanalyzed as the head of the \isi{Result phrase}; the structure is given in \REF{ex:lalu:55}.
...
In \ili{Swedish}, we can state that the heads in the \isi{vP} cluster up in a left-right order at the left edge of the \isi{vP}. Verbs in \ili{Swedish} today form a cluster, in which nothing generally intervenes (of course, here we exclude V in C):
...
In this analysis, the word order change is tied to a \isi{reanalysis} of the \isi{particle}. In fact, we can note a couple of other changes that occurred around the same time, which arguably are also a consequence of the change in the syntax of particles. Firstly, in older \ili{Swedish}, \isi{adverbial} particles could occur in double object constructions, as in \REF{ex:lalu:58}. This is no longer possible in present-day \ili{Swedish}, regardless of word order; compare \REF{ex:lalu:59} and \REF{ex:lalu:60}.
...
\gll  Ge     mig (*    hit)     en   skål.     \\
    give     me   {}    here   a     bowl\\\jambox*{(present-day \ili{Swedish})}}
...
This restriction can be explained by the assumption that the head responsible for the introduction of indirect objects competes for the same position (i.e., Res) as the present-day \ili{Swedish} \isi{particle} (see e.g. \citealt{Ramchand2008}).
...
\ex\label{ex:lalu:62}
\ea[]{
\gll  blev   in-satt   i   kyrkan (present-day \ili{Swedish})\\
...
Leaving the analysis of \isi{particle} \isi{incorporation} aside, we conclude that there are several reasons to assume that the word order change is a consequence of a change in the syntax of particles, rather than, for instance, in the general principles of linearization in \ili{Swedish} or the possibility of argument shifts. 
...
These cases with word order variation tend to involve clearly phrasal ‘particles’, like \textit{till fånga} in \REF{ex:lalu:62}, \textit{i ordning} ‘in order’, or \textit{färdigt} ‘ready’. We propose that they in fact still involve phrasal modifiers in present-day \ili{Swedish}, regardless of word order. In other words, the word order variability that we see in examples like \REF{ex:lalu:63} is a remnant of the older \ili{Swedish} pattern, which we also still find more generally in the other \ili{Germanic} languages.



In the next section, we briefly discuss later developments in the distribution of particles in \ili{Swedish} and conclude the paper.


\section{Further developments and conclusion}\label{sec:lalu:7}
In this paper, we have traced the development of the present-day \ili{Swedish} word order in \isi{particle} constructions, mainly in texts from the (Late) \ili{Modern Swedish} period. Unlike other significant changes in the history of \ili{Swedish} (most notably the shift from \isi{OV to VO}), the old word order seems to have been stable until the middle of the 17\textsuperscript{th} century, and the change is not shared with any of the other North \ili{Germanic} languages. We have suggested that the word order variation found in \ili{Old Swedish} (and modern \ili{Icelandic} and \ili{Norwegian}) is due to the branching of the Result modifier (the \isi{particle}) and a general shifting of pronouns (that we also see in \isi{object shift} across sentence adverbs). The present-day \ili{Swedish} word order, on the other hand, we propose is a consequence of a \isi{reanalysis} of the \isi{particle} as the head of \isi{ResP}; it is spelled out together with the other verbal heads and will always precede all verbal complements. 



The \isi{reanalysis} can first be detected in our data in Agneta Horn’s text (b. 1629), and the change was approaching its conclusion by the beginning of the 19\textsuperscript{th} century, at least if conservative texts are disregarded (see \figref{fig:lalu:2}) – the change thus largely took place during the \ili{Late Modern Swedish} period. We have further seen that not all particles and contexts behave alike. Adverbs, particularly in the context of a directional PP, are more reluctant to change. Salvius (b. 1706) is the first in our corpus who has the order \isi{particle}–object–PP. We have suggested that in older \ili{Swedish}, there was a preference for treating the \isi{particle}/\isi{adverb} as a modifier of the PP. This possibility still exists in present-day \ili{Swedish}, as evidenced from the example in \REF{ex:lalu:17b} above, repeated here as \REF{ex:lalu:64}.
...
The difference between \ili{Swedish} and \ili{Norwegian} is even more clear with the modifier \textit{helt} ‘completely’. Here, splitting the \isi{particle} and the modifier is the default strategy in \ili{Swedish}, while they must stay together in \ili{Norwegian}, surfacing after the object:
...
\gll  Jag  slet  ut       mig     helt         \\
    I     wore   out   me     completely \\\jambox*{(present-day \ili{Swedish})}}
\ex[??]{
...
It seems then that the possibility of treating the \isi{particle} as a head in the verb phrase emerged in the 17\textsuperscript{th} century and has continually gained ground since then. Today, a head analysis seems to be strongly preferred, whenever possible. However, the variation with regard, for example, to modified particles needs to be investigated further. Since examples are not very frequent in the corpora, it is hard to study whether this construction has changed over time in \ili{Swedish}. Another case where we find a strong preference for having an overt Res head is in cases of what we may call \isi{particle} doubling. Here, a ground-introducing preposition is doubled as a \isi{particle}, preceding the \isi{direct object}. As far as we know, these were not available at earlier stages of \ili{Swedish}, and they are strictly ungrammatical in the other Mainland North \ili{Germanic} languages. We give examples in \ili{Swedish} \REF{ex:lalu:70} and \ili{Norwegian} \REF{ex:lalu:71} below.
...
\begin{multicols}{2}
\begin{tabbing}
\isi{LCA} \hspace{1ex}\=  \isi{Linear Correspondence Axiom}\kill
\isi{EWL} \>  Elder Westrogothic law\\
\isi{LCA} \>  \isi{Linear Correspondence Axiom}\\
OP  \> Object--\isi{Particle} order\\
OV  \> Object–Verb order\\
PO  \> \isi{Particle}–Object order\\
UL  \> Law of Uppland\\
...
\item [Gyllenborg, Carl (b. 1679).] \textit{Swenska sprätthöken} [The \ili{Swedish} dandy]. Edited by Lennart Breitholtz \& Einar Törnqvist. Stockholm: Gebers, 1959. Available through FTB/\isi{Korp}. See \citet{MarttalaStromquist2001}. Original (from 1740) available through LB. 
...
\item[\normalfont LB:]    The \ili{Swedish} literature bank: \url{http://www.litteraturbanken.se}
\item[\normalfont SPF:]  \ili{Swedish} prose fiction 1800–1900. Available through \isi{Korp}.
\end{description}
